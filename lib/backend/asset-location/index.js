...
function updateAssetLocationSS(config) {
    ...
    function successCb(value) {
        logger.publishLog(global_config_1.GC.LOG_LEVEL.SUCCESS, 'updateAssetLocationSS - Succeeded ', value);
    }
    function failureCb(reason) {
        logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetLocationSS - Failed ', reason);
    }
    function updateAssetLocation(assetsOpts) {
        ...
        logger.publishLog(global_config_1.GC.LOG_LEVEL.DEBUG, 'DEBUG: ', 'updateAssetLocationSS - In Update Asset Location');
    ...
    function createAsset(assetID, assetData) {
        logger.publishLog(global_config_1.GC.LOG_LEVEL.DEBUG, 'DEBUG: ', 'updateAssetLocationSS - in Create Asset');
        var assetsCol = collection_lib_1.CbCollectionLib(global_config_1.CollectionName.ASSETS);
    ...
    function HandleMessage(err, msg, topic) {
        if (err) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetLocationSS -  Failed to wait for message: ', err, ' ', msg, '  ', topic);
        ...
        try {
            ...
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetLocationSS - Failed parse the message: ', e);
        ...
        if (!assetID) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetLocationSS - Invalid topic received: ', topic);
            return;
        }
        ...
        assetsCol
            ...
            if (data.DATA.length === 1) {
                ...
                logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'ERROR: ', "updateAssetLocationSS -  Asset doesn't exist so, ignoring: ", data);
            }
            else {
                logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'ERROR: ', 'updateAssetLocationSS -  Multiple Assets with same assetId exists: ', data);
            }
        })
            .catch(function (reason) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetLocationSS - Failed to fetch: ', reason);
    ...
    function WaitLoop(err, data) {
        if (err) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetLocationSS - Subscribe failed for: ', SERVICE_INSTANCE_ID, ': ', data);
            config.resp.error(data);
        }
        logger.publishLog(global_config_1.GC.LOG_LEVEL.SUCCESS, 'updateAssetLocationSS - Subscribed to Shared Topic. Starting Loop.');
