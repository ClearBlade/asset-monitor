...
function handleServiceSettings(settings) {
    if (!settings) {
        settings = global_config_1.GC.UPDATE_ASSET_LOCATION_SETTINGS;
    }
    return settings;
}
function updateAssetLocationSS(config) {
    ...
    function updateAssetLocation(assetsOpts) {
        ...
        var changes = {};
        for (var i = 0; i < config.settings.KEYS_TO_UPDATE.length; i++) {
            var curKey = config.settings.KEYS_TO_UPDATE[i];
    ...
    function createAsset(assetID, assetData) {
        ...
        try {
            ...
            logger.publishLog(global_config_1.LogLevels.ERROR, 'ERROR: ', SERVICE_INSTANCE_ID, ': Failed to stringify ', e);
