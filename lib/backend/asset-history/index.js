...
function createAssetHistorySS(config) {
    ...
    function successCb(value) {
        logger.publishLog(global_config_1.GC.LOG_LEVEL.SUCCESS, 'createAssetHistorySS - AssetHistory Creation Succeeded ', value);
    }
    function failureCb(reason) {
        logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'createAssetHistorySS - Failed ', reason);
    }
    ...
    function createCustomHistoryData(assetID, parsedMsg) {
        ...
        if (!customData) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.DEBUG, 'createAssetHistorySS - Custom Data Missing: ', customData);
            return [];
        }
        var historyData = [];
        var currDate = new Date().toISOString();
    ...
    function HandleMessage(err, msg, topic) {
        if (err) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'createAssetHistorySS - Failed to wait for message: ', err, ' ', msg, '  ', topic);
        ...
        try {
            ...
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'createAssetHistorySS - Failed parse the message: ', e);
            return;
        }
        var assetHistoryItems = [];
        var assetID = Util_1.getAssetIdFromTopic(topic);
        if (!assetID) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'createAssetHistorySS - Invalid topic received: ' + topic);
        }
        ...
        logger.publishLog(global_config_1.GC.LOG_LEVEL.DEBUG, 'createAssetHistorySS - HistoryData ', assetHistoryItems);
    ...
    function WaitLoop(err, data) {
        if (err) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'createAssetHistorySS - Subscribe failed for: ', SERVICE_INSTANCE_ID, ': ', data);
            config.resp.error(data);
        }
        logger.publishLog(global_config_1.GC.LOG_LEVEL.SUCCESS, 'createAssetHistorySS - Subscribed to Shared Topic. Starting Loop.');
