...
function createAssetHistorySS(config) {
    ...
    function successCb(value) {
        logger.publishLog(global_config_1.GC.LOG_LEVEL.SUCCESS, 'AssetHistory Creation Succeeded ', value);
    }
    ...
    function createCustomHistoryData(assetID, parsedMsg) {
        ...
        if (!customData) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.DEBUG, 'Custom Data Missing: ', customData);
            return [];
        }
        var historyData = [];
        var currDate = new Date().toISOString();
    ...
    function HandleMessage(err, msg, topic) {
        ...
        logger.publishLog(global_config_1.GC.LOG_LEVEL.DEBUG, 'HistoryData ', assetHistoryItems);
