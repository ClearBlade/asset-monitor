            ...
            var promise = rulesCollection
                ...
                var promise = eventsCollection
                    ...
                    for (var i = 0; i < eventData.DATA.length; i++) {
                        var hasOverlappingEntities = compareOverlappingEntities(eventData.DATA[i], splitEntities);
                        if (hasOverlappingEntities) {
                            shouldProceed_1 = true;
                            var eventHistoryCollection = collection_lib_1.CbCollectionLib(global_config_1.CollectionName.EVENT_HISTORY);
                            var query_1 = ClearBlade.Query().equalTo('id', eventData.DATA[i].id);
                            if (compoundEventTypes.indexOf(eventData.DATA[i].type) > -1) {
                                ...
                                if (eventData.DATA[i].state === 'Active and Acknowledged') {
                                    ...
                                    utils_1.publishToEventTopic(msg, eventData.DATA[i].type, 'closed', splitEntities);
                                    return Promise.all([
