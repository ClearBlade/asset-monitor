...
var compoundEventTypes = ['dau-external-alarm'];
function shouldCreateOrUpdateEvent(ruleID, splitEntities, eventTypeId) {
...
function closeRules(ids, splitEntities, timestamp) {
    if (ids.length) {
        ...
        var promise = Promise.all(ids.map(function (id) {
            ...
            var promise = rulesCollection
                ...
                var promise = eventsCollection
                    ...
                    return Promise.all(eventData.DATA.map(function (e) {
                        var hasOverlappingEntities = compareOverlappingEntities(e, splitEntities);
                        if (hasOverlappingEntities) {
                            shouldProceed_1 = true;
                            var eventHistoryCollection = collection_lib_1.CbCollectionLib(global_config_1.CollectionName.EVENT_HISTORY);
                            var query_1 = ClearBlade.Query().equalTo('id', e.id);
                            if (compoundEventTypes.indexOf(e.type) > -1) {
                                // bulky code but will make it easier to handle modifications
                                if (e.state === 'Active and Acknowledged') {
                                    // publish to closed topic
                                    var msg = ClearBlade.Messaging();
                                    utils_1.publishToEventTopic(msg, e.type, 'closed', splitEntities);
                                    return Promise.all([
                                        ...
                                        eventHistoryCollection.cbCreatePromise({
                                            item: {
                                                event_id: e.id,
                                                timestamp: timestamp || events_1.getDefaultTimestamp(),
