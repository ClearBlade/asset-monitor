...
function getOpenStateForEvent(eventTypeId) {
    ...
    var promise = eventTypesCollection.cbFetchPromise({ query: eventTypesCollectionQuery }).then(function (data) {
        var typeData = Array.isArray(data.DATA) && data.DATA[0];
        return typeData && !!typeData.has_lifecycle && JSON.parse(typeData.open_states || '[]').length ? JSON.parse(typeData.open_states)[0] : '';
    });
    Promise.runQueue();
    return promise;
}
