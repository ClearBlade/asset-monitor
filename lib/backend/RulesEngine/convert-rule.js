...
function getConditionPropsForFact(id, condition, isPartOfType) {
...
function formatConditionForEntity(id, condition, isPartOfType) {
...
function addConditions(ruleId, condition) {
    ...
    var promise;
    if (condition.entity.entity_type === types_1.EntityTypes.ASSET_TYPE || condition.entity.entity_type === types_1.EntityTypes.AREA_TYPE) {
...
function parseAndConvertConditions(ruleId, conditions) {
    ...
    var promise = Promise.all(subConditions.map(function (s) { return convertCondition(ruleId, s); })).then(function (rules) {
        if (rules.length > 1) {
            if (firstKey === 'and') {
                return {
