    ...
    switch (condition.entity.entity_type) {
        case types_1.EntityTypes.ASSET_TYPE:
            promise = async_1.getAllAssetsForType(condition.entity.id).then(function (assets) {
                var promiseTwo = Promise.all(assets.map(function (asset) { return formatConditionForEntity(asset.id, condition, true); })).then(function (results) {
        ...
        case types_1.EntityTypes.AREA_TYPE:
            promise = async_1.getAllAreasForType(condition.entity.id).then(function (areas) {
                var promiseTwo = Promise.all(areas.map(function (area) { return formatConditionForEntity(area.id, condition, true); })).then(function (results) {
                    for (var i = 0; i < results.length; i++) {
                        if (Array.isArray(results[i])) {
                            conditions.push.apply(conditions, results[i]);
                        }
...
function convertCondition(condition) {
    if (condition.entity) {
        ...
        var promise = parseAndConvertConditions(condition);
        Promise.runQueue();
        return promise;
    }
}
function parseAndConvertConditions(conditions) {
    var firstKey = Object.keys(conditions)[0];
    var subConditions = conditions[firstKey];
    var promise = Promise.all(subConditions.map(function (s) { return convertCondition(s); }))
        .then(function (rules) {
        if (rules.length > 1) {
            if (firstKey === 'and') {
                return {
