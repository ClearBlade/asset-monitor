...
function rulesEngineSS(_a) {
    ...
    function subscribeAndInitialize() {
        Promise.all(__spreadArrays(incomingDataTopics, [RULES_UPDATED_TOPIC, DurationEngine_1.DURATION_TOPIC]).map(function (topic) {
    ...
    function initializeWhileLoop() {
        ...
        while (true) {
            messaging.waitForMessage(__spreadArrays(incomingDataTopics, [RULES_UPDATED_TOPIC, DurationEngine_1.DURATION_TOPIC]), handleIncomingMessage);
        }
    }
    function handleIncomingMessage(err, msg, topic) {
        if (err) {
            ...
            else if (topic === DurationEngine_1.DURATION_TOPIC) {
                durationEngine.timerExecuted(err, msg);
            }
            else {
                var incomingData = void 0;
                try {
                    incomingData = __assign(__assign({}, JSON.parse(msg)), { entityType: topic.includes('_assets') ? types_1.EntityTypes.ASSET : types_1.EntityTypes.AREA });
                }
                catch (e) {
