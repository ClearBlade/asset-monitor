...
var engine = new RulesEngine_1.RulesEngine();
function rulesEngineApi(_a) {
    var resp = _a.resp, rules = _a.rules, incomingDataTopics = _a.incomingDataTopics;
    var messaging = ClearBlade.Messaging();
    Promise.all(rules.map(function (ruleData) {
        var promise = engine.addRule(ruleData)
    ...
    function handleIncomingMessage(err, msg, topic) {
        if (err) {
            ...
            engine.run(fact).then(function (successMsg) {
                //@ts-ignore
                log(successMsg);
            }).catch(function (e) {
                resp.error(e);
...
exports.rulesEngineApi = rulesEngineApi;
