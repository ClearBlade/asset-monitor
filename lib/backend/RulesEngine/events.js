...
function processEvent(ruleParams, entities, actionTopic, trigger) {
    ...
    var promise = async_1.closeRules(closesIds, splitEntities).then(function () {
        var promise = async_1.shouldCreateOrUpdateEvent(ruleID, splitEntities).then(function (shouldCreate) {
            if (shouldCreate) {
