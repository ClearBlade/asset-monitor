...
function processSuccessfulEvent(ids, ruleParams, entities, actionTopic, trigger) {
    if (timeframe_1.doesTimeframeMatchRule(getDefaultTimestamp(), ruleParams.timeframe)) {
...
function processEvent(ruleParams, entities, actionTopic, trigger) {
    ...
    var promise = async_1.closeRules(closesIds, splitEntities, ruleParams.timestamp).then(function (shouldProceed) {
        if (shouldProceed) {
            var promise_1 = async_1.shouldCreateOrUpdateEvent(ruleID, splitEntities, eventTypeID).then(function (shouldCreate) {
                if (shouldCreate) {
                    var promise_2 = async_1.getStateForEvent(eventTypeID).then(function (_a) {
                        ...
                        if (is_open) {
                            ...
                            utils_1.publishToEventTopic(msg, eventTypeID, 'open', splitEntities);
                        }
                        var id = uuid();
