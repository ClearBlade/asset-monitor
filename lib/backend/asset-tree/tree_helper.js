...
function getTree(treeID) {
    ...
    var promise = treeCol
        ...
        try {
            ...
            log('getTree::: ', tree_1.AssetTree.treeToString(tree));
...
function moveChild(parentID, childNode, childTreeID) {
    var destTree = getTreeIdForAsset(parentID).then(getTree);
    var srcTree = getTree(childTreeID);
...
function assetTreeHandler(req, resp, options) {
    ...
    function handleCreate(options) {
        ...
        var rootNode = options.NODES[rootNodeID];
        if (!rootNode) {
            resp.error('Root node is missing.');
        }
        var tree = new tree_1.AssetTree(rootNode, undefined, options.NODES);
        log('logging tree');
    ...
    function handleGetTree(options) {
        getTree(options.TREE_ID).then(successFn).then(failureFn);
    }
