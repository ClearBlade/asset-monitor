    ...
    AssetTypeTree.prototype.getTopLevelAssetTypes = function () {
        ...
        var callback = function (err, data) {
            if (err) {
                ...
                topLevelAssetTypes.push(data.DATA[0]);
            }
        };
        topLevelAssetTypesIDs.forEach(function (typeID) {
            var query = ClearBlade.Query({ collectionName: global_config_1.CollectionName.ASSET_TYPES }).equalTo('id', typeID);
            query.fetch(callback);
        });
        return topLevelAssetTypes;
    };
    AssetTypeTree.prototype.createAssetType = function (newAssetTypeID, newAssetType, children) {
            ...
            switch (options.METHOD_NAME) {
                ...
                case AssetTypeTreeMethod.CREATE_ASSET_TYPE:
                    if (options.ASSET_TYPE_ID && options.NEW_ASSET_TYPE && options.PARENTS && options.CHILDREN) {
                        assetTypeTree.createAssetType(options.ASSET_TYPE_ID, options.NEW_ASSET_TYPE, new Set(options.CHILDREN));
                    }
                    break;
