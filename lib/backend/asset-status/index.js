...
function updateAssetStatusSS(config) {
    ...
    function failureCb(reason) {
        logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetStatusSS - Failed ', reason);
    }
    function MergeAsset(assetID, msg) {
        ...
        var promise = assetsCol.cbFetchPromise({ query: assetFetchQuery }).then(function (data) {
            if (data.DATA.length <= 0) {
                logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetStatusSS - No asset found for id ', assetID);
                return Promise.reject(' No asset found for id ' + assetID);
            }
            if (data.DATA.length > 1) {
                logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetStatusSS - Multiple Assets found for id ', assetID);
            ...
            try {
                ...
                logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetStatusSS - Failed while parsing: ', e);
    ...
    function handleMessage(err, msg, topic) {
        if (err) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetStatusSS - Failed to wait for message: ', err, ' ', msg, '  ', topic);
        ...
        if (!assetID) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetStatusSS - Invalid topic received: ', topic);
    ...
    function WaitLoop(err, data) {
        if (err) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'updateAssetStatusSS - Subscribe failed ', data);
            config.resp.error(data);
        }
        logger.publishLog(global_config_1.GC.LOG_LEVEL.SUCCESS, 'updateAssetStatusSS - Subscribed to Shared Topic. Starting Loop.');
