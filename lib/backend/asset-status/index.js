...
function updateAssetStatusSS(config) {
    ...
    function MergeAsset(assetID, msg) {
        ...
        var promise = assetsCol.cbFetchPromise({ query: assetFetchQuery }).then(function (data) {
            if (data.DATA.length <= 0) {
                logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'No asset found for id ', assetID);
                return Promise.reject(' No asset found for id ' + assetID);
            }
            if (data.DATA.length > 1) {
                logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'Multiple Assets found for id ', assetID);
            ...
            try {
                ...
                logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'Failed while parsing: ', e);
    ...
    function WaitLoop(err, data) {
        if (err) {
            logger.publishLog(global_config_1.GC.LOG_LEVEL.ERROR, 'Subscribe failed ', data);
